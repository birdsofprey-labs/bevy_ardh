    ...
    for (mut q, pnute) in qry.iter_mut() {
        ...
        let qloop = q.stree.dfs(|node| {
           ...
           // return (node.depth as i32) < md.0 ;
            ...
            let sides = [Vec3::new(1.0, 0.0, 0.0), 
                                    Vec3::new(-1.0, 0.0, 0.0),
                                    Vec3::new(0.0, 0.0, 1.0), 
                                    Vec3::new(0.0, 0.0, -11.0)]
                                    .map(|t| t + node.tx.translation )
                                    .map(|t| );

            let tx = node.tx.translation+(node.tx.translation-cam_tx.translation).normalize_or_zero()*0.5*node.size;
            let boxy_dist1 =  (cam_tx.translation* Vec3::new(1.0, 1.0, 1.0)).distance(node.tx.translation * Vec3::new(1.0, 1.0, 1.0));
            let boxy_dist2 =  (cam_tx.translation* Vec3::new(1.0, 1.0, 1.0)).distance(tx * Vec3::new(1.0, 1.0, 1.0));
            let boxy_dist = boxy_dist1.min(boxy_dist2);
...
//q.stree.leafs.clear();
        ...
        for add_node in add_list {
            ...
            let mat = standard_materials.add(StandardMaterial { base_color,   .. default() });
