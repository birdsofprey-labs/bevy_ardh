...
fn setup_system(
   ...
   let sides_txes = [
    ...
    Transform::from_translation(Vec3::X * pushto).with_scale(Vec3::new(1.0, -1.0, 1.0))
        .mul_transform(  Transform::from_rotation(Quat::from_axis_angle(-Vec3::Z, 90f32.to_radians()) )),
        
    Transform::from_translation(Vec3::X * -pushto).with_rotation( Quat::from_axis_angle(Vec3::X, 90f32.to_radians()) ).with_scale(Vec3::new(-1.0, 1.0, -1.0))
        .mul_transform(  Transform::from_rotation(Quat::from_axis_angle(Vec3::Z, 90f32.to_radians()) )),
        

    Transform::default().with_translation(Vec3::Y * pushto)//.with_scale(Vec3::new(1.0, -1.0, 1.0))
            .mul_transform(  Transform::from_rotation(Quat::from_axis_angle(Vec3::Y,  0f32.to_radians()) )), // +y
   

    Transform::from_translation(Vec3::Z *-pushto)
        .mul_transform(  Transform::from_rotation(Quat::from_axis_angle(Vec3::X, -90f32.to_radians()) )),
    Transform::from_translation(Vec3::Z * pushto).with_scale(Vec3::splat(-1.0))
        .mul_transform(  Transform::from_rotation(Quat::from_axis_angle(Vec3::X,  90f32.to_radians()) )),
//     Transform::from_rotation(Quat::from_axis_angle(Vec3::Z,  90f32.to_radians()) )
//         .mul_transform( Transform::from_translation(Vec3::X * pushto) )
 ];
   for (mut face_idx,local_tx) in sides_txes.iter().enumerate() {
    ...
    ******************************************/
    ...
    //     id: 0,
    //     depth: 0,
    //     tx: Transform::default(),//.with_translation(Vec3::Y * 500.0),
    //     index: ZNodeIndex::None,
    //     uv_offset:  Vec2::new(0.0, 0.0), 
    //     uv_scale: 1.0,
    //     size: PLANET_SIZE
    ...
    // commands.spawn(ArdhFlat {
    // //    local_tx: Transform::from_rotation(Quat::from_axis_angle(Vec3::Z, 90f32.to_radians())).with_translation(Vec3::X * 250.0),
    // local_tx: Transform::from_translation(Vec3::X * pushto).mul_transform(  Transform::from_rotation(Quat::from_axis_angle(Vec3::Z, -90f32.to_radians()) )),
    // size: PLANET_SIZE,
    //     stree: ardh::ardh::SearchTree { tree , 
    //                     running: true, 
    //                     stack,
    ...
    commands.spawn((
        ...
        FogSettings {
            ...
            falloff:FogFalloff::from_visibility_color(2500.0, Color::MIDNIGHT_BLUE), //FogFalloff::Atmospheric { extinction: Vec3::splat(0.0075), inscattering: Vec3::splat(0.01) },
            ..default()
        },
       // GridShadowCamera,
    ...
    for (mut q, mut qbatch_assets, pnute) in qry.iter_mut() {
        ...
        let qloop = q.stree.dfs(|node| {
           ...
           // return (node.depth as i32) < md.0 ;
            ...
            let boxy_dist: f32 = [Vec3::new(0.0, 0.0, 0.0), 


            Vec3::new(1.0, 0.15, 1.0), 
            Vec3::new(1.0, 0.15, -1.0), 
            Vec3::new(-1.0, 0.15, 1.0), 
            Vec3::new(-1.0, 0.15, -1.0), 
            ...
            Vec3::new(0.0, 0.15, -1.0)]
                    .map(|t|  (local_tx.transform_point((node.tx.transform_point(t*node.size)) )))
                    .map(|t| cam_tx.translation.distance(t ) - node.size*0.0  ).iter().fold(10000000.0, |a,x| x.min(a));
                    //.sum::<f32>() / 4.0 ; // -50.0
            ...
            let cliper =  0.25 + 0.8*( 1000000.0/boxy_dist.powf(2.0) ).log2() ;
...
//q.stree.leafs.clear();
        ...
        for del_id in del_ids.iter() {
           // if del_ids.contains( &del_node.address ) {
                ...
                batch_assets.tiles_removal.push(*del_id);
           // }
        }
...
fn sdiver2(mut commands: Commands, 
   ...
   // mut materials: ResMut<Assets<CustomMaterial>>,
    ...
    mut md: ResMut<MaxDepth>, 
    // md: Res<MaxDepth>,
      mut qry: Query<(&mut ArdhFlat, &mut BatchAssets, Entity)>, tileq: Query<(Entity, &TileId)>, qcam: Query<&Transform, With<Camera>>) {
        ...
        let test_col = asset_server.load("testgrid.png");
    ...
    if let Some((mut q, mut qbatch_assets, pnute)) = qry.iter_mut().next()
    {
        for batch in qbatch_assets.queue.iter_mut() {
            ...
            // for del_ent in batch.tiles_removal.iter() {
            //     //if commands.get_entity(*del_ent).is_some() {
            //      commands.entity(*del_ent).despawn();
            // // }
            //  }
            ...
            for ((test_hgt, test_nor), add_node) in  batch.hgt_images.iter().zip(batch.nor_images.iter()).zip(batch.tiles.iter()) {
                let pushto = 2000.0;
                //let base_color = Color::hsl(rng.gen::<f32>()*360.0, 0.2+(rng.gen::<f32>()) * 0.5, 0.2+rng.gen::<f32>() * 0.5);
                let tile_mesh_aabb =  compute_aabb( &add_node.tx);
                let mat = materials.add(ExtendedMaterial {
                    base: StandardMaterial {
                        base_color: Color::DARK_GREEN,//*0.5 + base_color*0.5,// Color::DARK_GREEN,
                        specular_transmission: 0.2,
                        metallic: 0.025,
                        perceptual_roughness: 0.8,
            ...
            let del_ids =  batch.tiles_removal.iter().map(|n|*n).collect::<HashSet<_>>();
