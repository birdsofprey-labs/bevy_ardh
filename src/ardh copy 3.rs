        ...
        for u in sub_nodes {
            ...
            if dosubdiv || u < self.tree.nodes_list.len() + 4 {
                ...
                if dosubdiv {
                    ...
                    let tx: Transform = v.tx * v.tx.with_scale(nscale).with_translation(Vec3::new(-size, 0.0, size));
                    let node = Node { id: QT::index(v.id, index), tx, index, size, depth, parent_copy: parent_copy.clone() };
                    self.tree.set(v.id, index, Some(node));

                    let index = ZNodeIndex::NorthEast;
                    let tx = v.tx * v.tx.with_scale(nscale).with_translation(Vec3::new(size, 0.0, size));
                    let node = Node { id: QT::index(v.id, index), tx, index, size, depth, parent_copy: parent_copy.clone() };
                    self.tree.set(v.id, index, Some(node));
