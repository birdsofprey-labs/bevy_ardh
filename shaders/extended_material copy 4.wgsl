...
//vertex_normal = normalize(vertex_normal);
    ...
    let height: f32  = textureGather(1, height_texture, height_sampler,  vec2<f32>(  vertex.uv.x, vertex.uv.y )  ).r;
    // vertex.uv = vec2<f32>(0.5*(-0.5 + vertex.uv.x * 2.0), 0.5*(-0.5 +  vertex.uv.y * 2.0));
    //let base = hbase * (2000.0+height *200.0);
    //vertex.position = base;//(model * (vec4<f32>(vertex.position.xyz, 0.0))).xyz  + ;
