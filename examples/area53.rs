...
use bevy::{prelude::*, utils::HashSet, render::{RenderPlugin, settings::{WgpuSettings, WgpuFeatures, RenderCreation}, render_resource::{AsBindGroup, ShaderRef}, primitives::Aabb}, pbr::{wireframe::{WireframePlugin, WireframeConfig, WireframeColor, Wireframe}, ExtendedMaterial, MaterialExtension, OpaqueRendererMethod, ScreenSpaceAmbientOcclusionBundle, CascadeShadowConfigBuilder}, reflect::{TypeUuid, TypePath}, core_pipeline::experimental::taa::TemporalAntiAliasPlugin, ecs::query::WorldQuery};
...
pub fn compute_aabb(this: &Mesh) -> Option<bevy::render::primitives::Aabb> {
    let Some(bevy::render::mesh::VertexAttributeValues::Float32x3(values)) =
        this.attribute(Mesh::ATTRIBUTE_POSITION)
    else {
        return None;
    };
    Some(bevy::render::primitives::Aabb::from_min_max(Vec3::splat(-500.0), Vec3::splat(500.0)))
...
fn setup_system(
    ...
    let pmesh = (Mesh::from(shape::Plane { size: 150.0*4.0, subdivisions: 128*1 }))
        //.unwrap()
        .with_generated_tangents()
        .unwrap();
    
    let paabb = compute_aabb(&pmesh).unwrap();
   ...
   // wireframe_config.global = false;
    ...
    commands.spawn((
        ...
        FogSettings {
            ...
            falloff:FogFalloff::from_visibility_color(500.0, Color::MIDNIGHT_BLUE), //FogFalloff::Atmospheric { extinction: Vec3::splat(0.0075), inscattering: Vec3::splat(0.01) },
            ..default()
        },
       // GridShadowCamera,
    ...
    for (mut q, mut qbatch_assets, pnute) in qry.iter_mut() {
        ...
        let qloop = q.stree.dfs(|node| {
           ...
           // return (node.depth as i32) < md.0 ;
            ...
            Vec3::new(0.0, 0.5, -1.0)]
                    .map(|t| t*node.size*0.25*4.0 + node.tx.translation )
            ...
            let cliper =  0.25 + 0.8*( 80000.0/boxy_dist.powf(2.0) ).log2() ;
