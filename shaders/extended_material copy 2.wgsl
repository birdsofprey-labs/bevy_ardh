...
//vertex_normal = normalize(vertex_normal);
    ...
    let height: f32  = textureGather(1, height_texture, height_sampler, vertex.uv).r;
    vertex.position = mesh_functions::mesh_position_local_to_world(model, vec4<f32>(vertex.position, 1.0)).xyz + vec3<f32>(0.0,height*50.0 - 7.0,0.0);//(model * (vec4<f32>(vertex.position.xyz, 0.0))).xyz  + ;
